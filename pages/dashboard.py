...
# ---------- ANNOUNCEMENTS ----------
    ...
    programme_option=st.selectbox(
        "Choose an Option",
        [
            "View Programmes",
            "My Joined Programmes"
        ]
    )
    if programme_option=="View Programmes":
        ...
        if not related_issues:
            st.warning("No related issues found.")
            st.stop()
        issue=st.selectbox(
            "Related Issue",
            related_issues
        ...
        with st.form("join_programme_form",clear_on_submit=True):
            ...
            if submit:
                if not programmename.strip():
                    ...
                    st.error("⚠️ Please upload proof.")
                else:
                    try:
                        os.makedirs("proofs",exist_ok=True)
                        proof_path=os.path.join("proofs",proof.name)
                        with open(proof_path,"wb") as f:
                            f.write(proof.getbuffer())
                        new_data=pd.DataFrame({
                            "username":[username],
...
# ---------- PROFILE ----------
    ...
    if st.session_state.get("logged_in",False):
        ...
        try:
            ...
            st.stop()
        user_rows=users[
            users["username"].astype(str)==str(username)
        ]
...
# ---------- ACHIEVEMENTS ----------
    ...
    try:
        ...
        st.stop()
    user_issues=issues[issues["Name"]==username]
...
# ---------- GALLERY ----------
    ...
    if gallery_option=="View VoxLocal Gallery":
        ...
        if not os.path.exists(gallery_path):
            ...
            st.info(
                "Make sure the 'gallery' folder is in the same GitHub repository as your Streamlit application."
            )
            st.stop()
        categories=[
            folder
            ...
            if os.path.isdir(
                os.path.join(gallery_path,folder)
            )
        ]
        if not categories:
        ...
        issues=[
            ...
            if os.path.isdir(
                os.path.join(category_path,folder)
            )
        ]
        if not issues:
        ...
        images=[
            image
            for image in os.listdir(issue_path)
        ...
        st.info(
            "Add your gallery images to the gallery folder in your GitHub repository."
        )
